...
ROOT_MICRO_ALPHA = 0.7
ROOT_TREND_WEIGHT = 0.9
ROOT_MAX_TREND_SHIFT = 2.5
ROOT_REVERSION_WEIGHT = 0.6
ROOT_MAX_REVERSION_SHIFT = 3
ROOT_TAKE_TREND_WEIGHT = 2
ROOT_TAKE_EDGE = .45
...
class DynamicProduct(Product):
    ...
    def trend_bias(self, fast_ema: float, slow_ema: float) -> float:
        raw_trend_shift = (fast_ema - slow_ema) * ROOT_TREND_WEIGHT
        return clamp(
            raw_trend_shift / max(1e-9, ROOT_MAX_TREND_SHIFT),
            -1.0,
            1.0,
        )
    ...
    def fair_value(
        ...
        fair_value = 0.65 * fast_ema + 0.35 * slow_ema
        ...
        trend_bias = self.trend_bias(fast_ema, slow_ema)
        ...
        return (
            fair_value + trend_shift + reversion_shift,
            clamp(signal_strength, 0.0, 1.0),
            trend_bias,
        )

    def take_liquidity(self, fair_value: float, trend_bias: float) -> None:
        reservation_price = fair_value
        trend_adjustment = ROOT_TAKE_EDGE * ROOT_TAKE_TREND_WEIGHT * trend_bias
        buy_threshold = reservation_price - ROOT_TAKE_EDGE - trend_adjustment
        sell_threshold = reservation_price + ROOT_TAKE_EDGE + trend_adjustment
    ...
    def make_quotes(
        ...
        directional_push = signal_strength * .2
        reservation_price = fair_value

        bid_quote = floor(reservation_price - ROOT_BASE_HALF_SPREAD)
        ask_quote = ceil(reservation_price + ROOT_BASE_HALF_SPREAD)

        # Start from your model-driven quotes
        final_bid = bid_quote
        final_ask = ask_quote

        # 1) Competitive constraints: improve/join the market
        if best_bid is not None:
            bid_quote = max(bid_quote, best_bid + ROOT_MIN_EDGE)

        if best_ask is not None:
            ask_quote = min(ask_quote, best_ask - ROOT_MIN_EDGE)

        # 2) Edge constraints: do not get too close to the other side
        if best_ask is not None:
        ...
        # 3) Final sanity check: never cross your own market
        if bid_quote >= ask_quote:
            midpoint = (bid_quote + ask_quote) / 2
            bid_quote = int(midpoint - ROOT_MIN_EDGE)
            ask_quote = int(midpoint + ROOT_MIN_EDGE)
    ...
    def build_orders(self) -> List[Order]:
        ...
        fair_value, signal_strength, trend_bias = self.fair_value(
            history, best_bid, best_ask, bid_volume, ask_volume
        )
        self.take_liquidity(fair_value, trend_bias)

        best_bid, bid_volume, best_ask, ask_volume = self.best_bid_ask()
        fair_value, signal_strength, _ = self.fair_value(
            history, best_bid, best_ask, bid_volume, ask_volume
        )
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if ASH_COATED_OSMIUM in state.order_depths:
            orders[ASH_COATED_OSMIUM] = StaticProduct(state, trader_state).build_orders()
