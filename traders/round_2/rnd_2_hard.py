...
class DynamicProduct(Product):
    ...
    def trend_shift(self, fast_ema: float, slow_ema: float) -> float:
        return clamp(
    ...
    def fair_value(
        ...
        trend_shift = self.trend_shift(fast_ema, slow_ema)
...
class Trader:
    ...
    def bid(self, state: Optional[TradingState] = None) -> int:
        if state is None:
            return 0
        ...
        product = DynamicProduct(dynamic_symbol, state, trader_state)
        best_bid, bid_volume, best_ask, ask_volume = product.best_bid_ask()

        if best_bid is None or best_ask is None or best_bid >= best_ask:
            return 0

        history = product.parse_history()
        mid_price = (best_bid + best_ask) / 2
        history.append(mid_price)
        history = history[-ROOT_HISTORY_LENGTH:]

        fair_value, signal_strength = product.fair_value(
            history,
            best_bid,
            best_ask,
            bid_volume,
        ...
        imbalance = compute_order_book_imbalance(bid_volume, ask_volume)
        fair_edge = fair_value - mid_price
        spread = best_ask - best_bid

        normalized_edge = clamp(fair_edge / max(ROOT_TAKE_EDGE, 1e-9), -1.0, 1.0)
        spread_penalty = clamp((spread - 1) / max(1.0, ROOT_BASE_HALF_SPREAD), 0.0, 1.0)
        history_confidence = clamp(len(history) / ROOT_HISTORY_LENGTH, 0.0, 1.0)

        opportunity_score = (
            0.45 * signal_strength
            + 0.25 * max(0.0, normalized_edge)
            + 0.20 * max(0.0, imbalance)
            + 0.10 * history_confidence
            - 0.20 * spread_penalty
        )
        return round(100 * clamp(opportunity_score, 0.0, 1.0))
