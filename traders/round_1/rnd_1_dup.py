...
class StableProduct(Product):
    ...
    def fair_value(
        ...
        if bid_price is not None and ask_price is not None and bid_price < ask_price:
            ...
            fair_value = 0.25 * ASH_FAIR_VALUE + 0.75 * mid_price
    ...
    def take_liquidity(self, fair_value: float) -> None:
        ...
        buy_threshold_agg = mid_price - 1.5
        buy_threshold_med = mid_price
        buy_threshold_light = mid_price + 2.5
        sell_threshold_agg = mid_price + 1.5
        sell_threshold_med = mid_price
        sell_threshold_light = mid_price - 2.5

        for ask_price, ask_volume in self.ordered_sells():
            if ask_price <= floor(buy_threshold_agg):
                self.add_buy(ask_price, ask_volume)
            
            elif ask_price <= floor(buy_threshold_med):
                self.add_buy(ask_price, min(ask_volume, 6))

            elif ask_price <= floor(buy_threshold_light):
                self.add_buy(ask_price, min(ask_volume, 3))

        for bid_price, bid_volume in self.ordered_buys():
            if bid_price >= ceil(sell_threshold_agg):
                self.add_sell(bid_price, bid_volume)
            
            elif bid_price >= ceil(sell_threshold_med):
                self.add_sell(bid_price, min(bid_volume, 6))

            elif bid_price >= ceil(sell_threshold_light):
                self.add_sell(bid_price, min(bid_volume, 3))
    ...
    def make_quotes(
        ...
        if best_bid is not None:
            bid_quote = max(bid_quote, best_bid + ASH_MIN_EDGE)
        if best_ask is not None:
            ask_quote = min(ask_quote, best_ask - ASH_MIN_EDGE)

        if best_ask is not None:
            bid_quote = max(bid_quote, best_ask - ASH_MIN_EDGE)
        if best_bid is not None:
            ask_quote = min(ask_quote, best_bid + ASH_MIN_EDGE)
